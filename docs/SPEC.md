...
| `API_DOCS_ENABLED` | *(unset)* | Swagger / ReDoc / `openapi.json`. Unset follows the environment — on in development, off in production. Set `true` on a server only for tunnelled access; nginx 404s the docs paths |
